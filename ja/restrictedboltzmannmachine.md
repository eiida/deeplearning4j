...
制限付きボルツマンマシンは比較的シンプルなので、まずはこのニューラルネットワークから見ていきましょう。以下の段落では、図と簡単な説明で、制限付きボルツマンマシンがどのように機能するのかを解説していきます。

RBMとは浅い2層のニューラルネットであり、ディープビリーフネットワークの構成要素です。RBMの最初の層は可視層または入力層と呼ばれ、2つ目の層は隠れ層と呼ばれます。

![Alt text](../img/two_layer_RBM.png)

上記の図の各円はニューロンに似たユニットを表したもので、ノードと呼ばれます。ノードとは、単に計算が行われる場所のことを指します。このノードは違う層のノードとのみ接続し、同じ層内のノード間では接続を持ちませ。

つまり層内でのコミュニケ―ションはないのです。これが、制限付きボルツマンマシンでいう**制限**に当たります。各ノードは入力を処理するための計算をする場所であり、その入力を伝えるか否かの[確率的](../glossary.html#stochasticgradientdescent)決定を行うことから始めます。（確率的とは「ランダムに決定した」という意味で、この場合、入力値を調整する係数がランダムに初期化されていることを指します。）
...
では、まずはひとつの画素値xを、2つの層にわたって見ていきましょう。隠れ層のノード1では、xに**重み**を掛け、それに**バイアス**と呼ばれるものを足します。この2つの計算結果が、活性化関数に入れられます。活性化関数はノードの出力値、つまり活性化関数を通過する、入力値xの信号の強さを生成します。

    活性化関数 f((重み w * 入力 x) + バイアス b ) = 出力 a

![Alt text](../img/input_path_RBM.png)

次に、複数の入力が隠れノードでどのように組み合わされるのか見てみましょう。各画素値xに個別の重みを掛けた結果を合計したものにバイアスを足した値が活性化関数を通過しノードの出力値を生成します。

![Alt text](../img/weighted_input_RBM.png)

全ての可視ノードからの入力は全ての隠れノードに渡されますから、RBMは**対称二部グラフ**と定義できます。

対称とは、各可視ノードに隠れノードが1つ接続されている様子を表しています（下記参照）。二部とは、2つの部分に分かれている、つまり2つの層を持つということで、グラフとはノードとノードの結合関係を表す数学的な用語です。

各隠れノードでは、各入力値xにそれぞれの重みwが掛けられます。つまり下記の例では、1つの入力値xが3つの重みを持っているため、重みは全部で12個あることになります（4つの入力ノード × 3つの隠れノード）。2層間の重みで行列を作ると、常に行は入力ノードと等しく、列は常に出力ノードと等しくなります。

各隠れノードは、それぞれ重みが掛けられた4つの入力値を受け取ります。これらの積の合計に次はバイアスを足し（少なくとも何かしらの活性化が起こるようになっている）、その演算結果が活性化アルゴリズムを通過し、各隠れノードに出力値を1つ生成します。

![Alt text](../img/multiple_inputs_RBM.png)

この2つの層がより層の深いニューラルネットワークの一部だった場合、隠れ層1の出力値は隠れ層2の入力値として渡され、そこから先は最後の分類層に達するまでいくつでも隠れ層を通過します。（シンプルなフィードフォワードの動きにするため、RBMのノードはオートエンコーダとしてのみ機能します。）

![Alt text](../img/multiple_hidden_layers_RBM.png)

## <a name="reconstructions">復元</a>

今回の制限付きボルツマンマシンのイントロダクションでは、このネットワークが教師なし学習法（教師なしとは、テストセットにデータに提供されたラベルを用いないこと）で、可視層と隠れ層1との間だけでそれ以上の層を通過せず前方パスと後方パスでどのようにデータの復元を学習するかを見ていきましょう。

再構成のフェーズでは、隠れ層1の活性化は後方パスでは入力になります。xの重みが前方パスで調整されたように、後方パスでは各ノード間のエッジに同じ重みが掛けられます。各可視ノードで、これらの積の合計に可視層のバイアスが足され、これらの計算結果が再構成されたものとなります。つまり、元々の入力値の近似値となります。これを下記の図で表すことができます。

![Alt text](../img/reconstruction_RBM.png)

RBMの重みはランダムに初期化されているため、再構成で得られた値と元々の入力値の差異が大きくなることが多いです。復元の誤差はrの値と入力値の違いだと考えることができます。反復学習のプロセスの中で、この誤差は最小になるまでRBMの重みを使って繰り返し逆伝播します。

逆伝播についてさらに詳細な説明を見たい方は[こちら](ja-neuralnet-overview.html#forward)。

このように前方パスでは、RBMは入力により、ノードの活性化、あるいは [重みxの出力値の確率](https://ja.wikipedia.org/wiki/%E3%83%99%E3%82%A4%E3%82%BA%E3%81%AE%E5%AE%9A%E7%90%86)、つまり`p(a|x; w)`の推測を行います。

しかし、後方パスでは、活性化関数が導入され、復元つまり元データの予測結果が出されると、RBMは与えられた活性化関数`a`（前方パスで用いられたのと同じ係数で重み付けされている）から入力値`x`の確率を算出しようとします。この2つ目のフェーズは、`p(x|a; w)` のように表されます。

これら2つの推測により入力xと活性化`a`つまり`p(x, a)`の同時確率分布が導き出されます。

この復元は、多くの入力に基づいて連続値を算出する回帰や、与えられた入力例に当てはまる個々のラベルを推測する分類とは異なった動きをします。

この復元とは元々の入力値の確率分布を推測する（つまり多くの異なる点の値を同時に推測する）ものです。これは[生成型学習](http://cs229.stanford.edu/notes/cs229-notes2.pdf)として知られるもので、分類で実行されるいわゆる識別学習とは区別しなければなりません。識別学習は、入力をラベルにマッピングし、データ点を効果的にグループに分ける手法です。

ここでは、入力データと復元を部分的に重なるが形の異なる正規曲線であると考えてみましょう。

推定された確率分布と真の入力値の分布の距離を測るために、RBMでは[Kullback Leiblerダイバージェンス](https://www.quora.com/What-is-a-good-laymans-explanation-for-the-Kullback-Leibler-Divergence)を使用します。数学的な詳しい説明は[Wikipedia](https://ja.wikipedia.org/wiki/%E3%82%AB%E3%83%AB%E3%83%90%E3%83%83%E3%82%AF%E3%83%BB%E3%83%A9%E3%82%A4%E3%83%96%E3%83%A9%E3%83%BC%E6%83%85%E5%A0%B1%E9%87%8F)を参照してください。

Kullback Leiblerダイバージェンスは、2つの曲線について、重ならずに相違（ダイバージェンス）している領域を測ります。RBMの最適化アルゴリズムは、隠れ層1の活性化により共有の重みが掛けられると元の入力値の近似値を算出できるよう、この2つの領域の差を最小化しようとします。グラフの左側が、実際の入力値による確率分布pで、その右が復元された分布qです。下のグラフはその差の積分です。

![Alt text](../img/KL_divergence_RBM.png)

2つの確率分布の差に基づいて重みを繰り返し調整することによって、RBMは、元データに近似させることを学習します。つまり、重みが次第に最初の隠れ層の活性化でエンコードされた入力データの構造を反映するようになります。その学習過程は、2つの確率分布が徐々に1つのグラフに収束していく過程のように見えます。
